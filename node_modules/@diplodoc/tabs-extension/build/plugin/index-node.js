...
// node_modules/@diplodoc/utils/build/cjs/common/index.js
var require_common = __commonJS({
  "node_modules/@diplodoc/utils/build/cjs/common/index.js"(exports2, module2) {
    "use strict";
    ...
    __export2(index_exports, {
      ...
      createIDGeneratorByStrategy: () => createIDGeneratorByStrategy2,
    ...
    var _key, _pending, _isInsideQuotation, _didQuotationClosed, _currentKeyType, _selectors, _handlers, _a;
    var AttrsParser = (_a = class {
      constructor(value) {
        __privateAdd(this, _key);
        __privateAdd(this, _pending);
        __privateAdd(this, _isInsideQuotation);
        __privateAdd(this, _didQuotationClosed);
        __privateAdd(this, _currentKeyType);
        ...
        __privateSet(this, _selectors, {
          id: /#/,
          class: /\./,
          attr: /[a-zA-Z-_]/
        });
        __privateSet(this, _handlers, Object.entries(__privateGet(this, _selectors)));
        if (value) {
          this.parse(value);
        }
      }
      ...
      type(of) {
        var _a2;
        return (_a2 = __privateGet(this, _handlers).find(([_, regex]) => regex.test(of))) == null ? void 0 : _a2[0];
      }
    ...
    }, _key = new WeakMap(), _pending = new WeakMap(), _isInsideQuotation = new WeakMap(), _didQuotationClosed = new WeakMap(), _currentKeyType = new WeakMap(), _selectors = new WeakMap(), _handlers = new WeakMap(), _a);
    ...
    var getQueueStore = (queueKey) => {
      ...
      return ((_a2 = window[QUEUES_SYMBOL]) == null ? void 0 : _a2[queueKey]) || false;
    };
    ...
    function createIDGeneratorByStrategy2(strategy = "random") {
      switch (strategy) {
        case "deterministic":
...
var import_utils6 = __toESM(require_common());
...
var PUNCTUATION_OR_SPACE_REGEXP = /^[\p{P}\s]$/u;
var PRESERVED_PUNCTUATION = /* @__PURE__ */ new Set(["#"]);
function encodeUnsupportedCodePoints(value) {
  return Array.from(value).map((character) => {
    if ((0, import_slugify.default)(character, SLUG_OPTIONS)) {
      return character;
    }
    if (PUNCTUATION_OR_SPACE_REGEXP.test(character) && !PRESERVED_PUNCTUATION.has(character)) {
      return character;
    }
...
function createTabKey(value) {
  const normalized = value.normalize("NFC").trim();
  const slug = (0, import_slugify.default)(encodeUnsupportedCodePoints(normalized), SLUG_OPTIONS);
  if (slug) {
    return slug;
  }
  return Array.from(normalized).map((character) => {
    var _a;
    return `u${(_a = character.codePointAt(0)) == null ? void 0 : _a.toString(16)}`;
  }).join("-");
}
...
function getContentMap(tokens) {
  let firstMap = null;
  let lastMap = null;
  tokens.forEach(({ map }) => {
    if (map) {
      firstMap = firstMap != null ? firstMap : map;
      lastMap = map;
    }
  });
...
var regular = (tabs, state, { containerClasses, tabsGroup, runId, variant: orientation, generateID }) => {
  var _a, _b;
  const tabsTokens = [];
  const tabListTokens = [];
...
var radio = (tabs, state, { containerClasses, tabsGroup, runId, generateID }) => {
  var _a, _b;
  const tabsTokens = [];
  const tabsOpen = new state.Token("tabs_open", "div", 1);
  const tabsClose = new state.Token("tabs_close", "div", -1);
...
var dropdown = (tabs, state, { containerClasses, tabsGroup, runId, generateID }) => {
  var _a, _b;
...
var accordion = (tabs, state, { containerClasses, tabsGroup, runId, generateID }) => {
  var _a, _b;
  const tabsTokens = [];
  const tabsOpen = new state.Token("tabs_open", "div", 1);
  const tabsClose = new state.Token("tabs_close", "div", -1);
...
function props(content, generateID) {
...
function tryToFindTabs(tokens, index) {
  ...
  const afterEndlistIdx = closeTokenIndex + 3;
  ...
  if (closeTokenLevel > openTokenLevel) {
    ...
    if ((tokenAfter == null ? void 0 : tokenAfter.type) === "list_item_close" && ((_a = tokens[afterEndlistIdx + 1]) == null ? void 0 : _a.type) === "bullet_list_close") {
...
function transform({
  ...
  return function tabs(md, options) {
    ...
    const generateID = externalGenerateID != null ? externalGenerateID : (0, import_utils6.createIDGeneratorByStrategy)("random");
    const plugin = (state) => {
      ...
      if (tabsAreInserted) {
        ...
        if (bundle && onBundle2) {
          onBundle2({ runtimeJsPath, runtimeCssPath, output }, env.bundled);
        }
      }
    };
    try {
...
// src/plugin/index-node.ts
var onBundle = (args, cache) => {
  copyRuntimeFiles(args, cache);
};
function transform2(options = {}) {
  return transform({ onBundle, ...options });
}
