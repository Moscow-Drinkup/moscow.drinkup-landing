"use strict";
const utils_1 = require("@diplodoc/utils");
...
const isMathBlockOrder = (src, pos) => checkCharsOrder(mathBlockOrder, src, pos);
...
const notEscaped = (src, pos) => !isEscaped(src, pos);
...
const skipInlineCode = (src, pos, max) => {
...
const skipInlineMath = (src, pos, max) => {
 ...
 */
function parseTableAttrs(state, iter) {
    const options = {};
    const { src } = state;
    ...
    while (parsing) {
        ...
        const result = (0, utils_1.parseMdAttrs)(state.md, src, first + 2, lineEnd);
        if (!result) {
            parsing = false;
            continue;
        }
...
function getTableRowPositions(state, startPosition, endPosition, startLine, opts = {}) {
    let endOfTable = null;
    let tableLevel = 0;
    let currentRow = [];
    let colStart = null;
    let pendingCellAttrs = {};
    let rowStart = null;
    let currentRowAttrs = {};
    ...
    while (iter.pos <= endPosition) {
        ...
        if (isRowOrder(state.src, iter.pos)) {
            ...
            if (insideRow) {
                ...
                iter.next(rowStartOrder.length);
                rowStart = iter.line;
                const hasRowAttrs = iter.pos <= iter.lineEnds &&
                ...
                if (hasRowAttrs) {
                    while (iter.pos <= iter.lineEnds &&
                        state.md.utils.isSpace(state.src.charCodeAt(iter.pos))) {
                        iter.next(1);
                    }
                }
                if (iter.pos <= iter.lineEnds &&
                    state.src.charCodeAt(iter.pos) === colonChar &&
...
function parseInlineAttrs(state, iter, prefixLen) {
    const result = {};
    const parsed = (0, utils_1.parseMdAttrs)(state.md, state.src, iter.pos + prefixLen, iter.lineEnds);
    if (!parsed) {
        return result;
    }
...
function extractAttributes(state, pos) {
    ...
    const attrsParser = new utils_1.AttrsParser();
 ...
 * @returns {void}
 */
function extractAndApplyClassFromToken(contentToken, tdOpenToken, log) {
    var _a;
    ...
    const attrs = new utils_1.AttrsParser().parse(allAttrs[0].trim());
    ...
    if (attrsClass) {
        if (attrsClass.split(' ').some((c) => c.startsWith('cell-align-'))) {
            log === null || log === void 0 ? void 0 : log.warn(`Deprecated: use align="..." inside cell attributes "::{align=\\"...\\"}" instead of .cell-align-* class in cell content "{ }"`);
        }
...
function parseHeaderRows(rawAttrs, log) {
    if (!('header-rows' in rawAttrs))
        return null;
    const raw = rawAttrs['header-rows'];
    const trimmed = raw.trim();
    const parsed = Number.parseInt(trimmed, 10);
    if (!Number.isInteger(parsed) || parsed <= 0 || String(parsed) !== trimmed) {
        log === null || log === void 0 ? void 0 : log.warn(`Invalid table header-rows value: "${raw}"`);
        return null;
    }
    return parsed;
}
function applyCellAttrs(token, rawAttrs, log) {
    if ('align' in rawAttrs) {
        const value = rawAttrs['align'];
        if (!VALID_ALIGN_VALUES.has(value)) {
            log === null || log === void 0 ? void 0 : log.warn(`Unknown table cell align value: "${value}"`);
        }
...
const yfmTable = (md, opts) => {
    md.block.ruler.before('code', pluginName, (state, startLine, endLine, silent) => {
        ...
        const { rows, endOfTable, pos, tableAttrs } = getTableRowPositions(state, startPosition, endPosition, startLine, opts);
        ...
        const headerRows = parseHeaderRows(tableAttrs, opts === null || opts === void 0 ? void 0 : opts.log);
        if (headerRows !== null) {
        ...
        for (let i = 0; i < rows.length; i++) {
            const { start: rowLineStarts, end: rowLineEnds, cols, rawAttrs: rowRawAttrs, } = rows[i];
            cellsMap.push([]);
            contentMap.push([]);
            ...
            for (let j = 0; j < cols.length; j++) {
                ...
                applyCellAttrs(token, cellRawAttrs, opts === null || opts === void 0 ? void 0 : opts.log);
                ...
                extractAndApplyClassFromToken(contentToken, rowTokens[rowTokens.length - 1], opts === null || opts === void 0 ? void 0 : opts.log);
            }
