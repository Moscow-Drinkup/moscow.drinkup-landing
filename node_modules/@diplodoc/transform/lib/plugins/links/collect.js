...
const collect = (input, options) => {
    ...
    const md = new markdown_it_1.default().use(index_1.default, { ...options, transformLink, needSkipLinkFn });
    ...
    while (i < tokens.length) {
        if (tokens[i].type === 'inline') {
            ...
            while (j < childrenTokens.length) {
                ...
                const href = (0, utils_2.getHrefTokenAttr)(linkToken);
                const isIncludeLink = (0, utilsFS_1.resolveRelativePath)(startPath, href).includes(`_includes${path_1.sep}`);
                if (!href || !(0, utils_2.isLocalUrl)(href) || isIncludeLink) {
                    j++;
                    continue;
                }
                const { pathname, hash } = (0, utils_1.parseHref)(href);
                if (pathname) {
                    const isPageFile = utils_2.PAGE_LINK_REGEXP.test(pathname);
                    if (isPageFile) {
