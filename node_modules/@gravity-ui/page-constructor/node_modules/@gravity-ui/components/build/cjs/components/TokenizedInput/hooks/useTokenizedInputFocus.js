...
exports.useTokenizedInputFocus = void 0;
const React = __importStar(require("react"));
const getInitialFocus = (tokens, fields, autoFocus) => {
    if (!autoFocus) {
        return undefined;
    }
...
const useTokenizedInputFocus = ({ fields, inputInfo, autoFocus, }) => {
    ...
    const [focus, setFocus] = React.useState(getInitialFocus(tokens, fields, autoFocus));
    const tokensRef = React.useRef(tokens);
    tokensRef.current = tokens;
    const onFocus = React.useCallback((newFocus) => {
        const currentTokens = tokensRef.current;
        const { idx, key, offset, ignoreChecks } = newFocus;
        const isNewToken = (idx === currentTokens.length && currentTokens[idx - 1]?.kind === 'new') ||
            (idx === currentTokens.length + 1 && currentTokens.at(-1)?.kind !== 'new');
        // new token is being finalized and not all fields are empty
        if (isNewToken) {
            const hasNonEmptyFields = Object.values(currentTokens.find((t) => t.kind === 'new')?.value ?? {}).some(Boolean) || ignoreChecks;
        ...
        setFocus((cur) => {
            ...
            const allValuesNonEmptyCondition = fields.some((f) => !currentTokens[cur.idx]?.value?.[f.key]);
            const curEmptyFieldCondition = idx === cur.idx && curKeyIndex < keyIndex && curValuesNonEmptyCondition;
            const nextEmptyFieldCondition = idx > cur.idx &&
    ...
    const getFocusRules = React.useCallback((value) => {
...
exports.useTokenizedInputFocus = useTokenizedInputFocus;
//# sourceMappingURL=useTokenizedInputFocus.js.map
