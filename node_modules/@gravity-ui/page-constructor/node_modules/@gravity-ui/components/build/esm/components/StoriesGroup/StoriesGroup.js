...
import { IndexType, StoriesLayout } from "../Stories/components/StoriesLayout/StoriesLayout.js";
import { block } from "../utils/cn.js";
import { StoriesPreview } from "./components/index.js";
import "./StoriesGroup.css";
const DEFAULT_MAX_SLIDER_ITEMS_COUNT = 12;
const b = block('stories-group');
// eslint-disable-next-line complexity
export const StoriesGroup = ({ open, groups, onItemSelect, disableOutsideClick = true, initialStoryIndex = [0, 0], maxSliderItemsCount = DEFAULT_MAX_SLIDER_ITEMS_COUNT, onClose, }) => {
    ...
    const onGroupSelect = React.useCallback((newGroupIndex) => {
    ...
    const indexType = (groups.length === 1 && currentItems.length === 1 && IndexType.Single) ||
        (groupIndex === 0 && itemIndex === 0 && IndexType.Start) ||
        (groupIndex === groups.length - 1 &&
            itemIndex === currentItems.length - 1 &&
            IndexType.End) ||
        IndexType.InProccess;
    return (_jsxs(Modal, { open: open, onClose: handleClose, disableOutsideClick: disableOutsideClick, className: b(), contentClassName: b('modal-content'), children: [_jsx(StoriesPreview, { maxSliderItemsCount: maxSliderItemsCount > 0 ? maxSliderItemsCount : 1, groups: groups, groupIndex: groupIndex, onGroupSelect: onGroupSelect, onClose: disableOutsideClick ? undefined : handleClose }), _jsx(StoriesLayout, { storyIndex: itemIndex, items: currentItems, indexType: indexType, handleButtonClose: handleButtonClose, handleGotoNext: handleGotoNext, handleGotoPrevious: handleGotoPrevious })] }));
};
//# sourceMappingURL=StoriesGroup.js.map
