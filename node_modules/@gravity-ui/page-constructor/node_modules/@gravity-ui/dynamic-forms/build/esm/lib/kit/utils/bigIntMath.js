...
export const multiply = (a, b, afterDotLength) => {
    try {
        ...
        const fractionalPartZeros = digitCapacityDegree > 0
            ? '0'.repeat(digitCapacityDegree - String(fractionalPart || '').length)
            : '';
        const stringFractionalPart = fractionalPartZeros + fractionalPart;
        const slicedFractionalPart = discardRightZeros(stringFractionalPart.slice(0, afterDotLength));
...
export const divide = (a, b, afterDotLength) => {
    try {
        const [aBefore, aAfter = ''] = a.split('.');
        const [bBefore, bAfter = ''] = b.split('.');
        const MIN_RESERVE = 20;
        const reserve = (afterDotLength && afterDotLength > MIN_RESERVE ? afterDotLength : MIN_RESERVE) +
            b.length;
        const digitCapacityDegree = reserve + aAfter.length - bAfter.length;
        const digitCapacity = mathPow(digitCapacityDegree);
        const divided = (BigInt(`${aBefore}${aAfter}`) * mathPow(reserve)) / BigInt(`${bBefore}${bAfter}`);
