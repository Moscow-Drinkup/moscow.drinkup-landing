...
export const useInitial = (value, spec, deps = []) => {
        ...
        if (viewType) {
            if (correctValue) {
                if (BigInt(scale[viewType].factor) > BigInt(scale[initialType].factor)) {
                    initialValue = divide(value, divide(scale[viewType].factor, scale[initialType].factor), 2);
                }
                else {
                    initialValue = multiply(value, divide(scale[initialType].factor, scale[viewType].factor), 2);
                }
            }
        ...
        if (correctValue) {
            Object.keys(scale).forEach((key) => {
                if (BigInt(scale[key].factor) > BigInt(scale[initialType].factor)) {
                    const candidate = divide(value, divide(scale[key].factor, scale[defaultType].factor), 2);
