...
function addFormatSection(sections, token, t, lang) {
    if (!token) {
        return;
    }
    ...
    sections.push(Object.assign(Object.assign({}, sectionConfig), { format: token, placeholder: getSectionPlaceholder(sectionConfig, token, t), options: getSectionOptions(sectionConfig, token, lang), hasLeadingZeros }));
}
...
function getSectionOptions(section, token, lang) {
    switch (section.type) {
        case 'month': {
            const format = section.contentType === 'letter' ? token : 'MMMM';
            let date = (0, date_utils_1.dateTime)({ lang }).startOf('year');
            const options = [];
            for (let i = 0; i < 12; i++) {
        ...
        case 'weekday': {
            ...
            let date = (0, date_utils_1.dateTime)({ lang }).day(0);
            const options = [];
            for (let i = 0; i < 7; i++) {
                options.push(date.format(format).toLocaleUpperCase());
                date = date.add(1, 'day');
