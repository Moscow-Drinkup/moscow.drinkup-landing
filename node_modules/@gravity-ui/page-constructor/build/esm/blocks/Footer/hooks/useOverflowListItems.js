...
import throttleFn from "lodash/throttle.js";
...
export function useOverflowListItems({ isDropdown = false, containerRef, items, itemSelector, moreButtonWidth = 0, }) {
    const [containerWidth, setContainerWidth] = React.useState(0);
    const [itemWidths, setItemWidths] = React.useState([]);
    const [itemGap, setItemGap] = React.useState(0);
    ...
    const updateItemWidths = React.useCallback(() => {
        ...
        const itemElements = Array.from(containerRef.current.querySelectorAll(itemSelector));
    ...
    const updateContainerSize = React.useMemo(() => throttleFn(() => {
...
function transformItemsToDropdownMenuItems(items) {
    return items.map((item) => ({
        text: item.text,
        href: item.url,
    }));
}
//# sourceMappingURL=useOverflowListItems.js.map
