import { ARROW_SIZE, getSameGroupArcOutset } from "./edgeGeometry.js";
...
export const KIND_ORDER = ['block', 'sub-block', 'component'];
const BAND_LABELS = {
    block: 'Blocks',
...
function createLabelMeasurer() {
    ...
    return (label) => Array.from(label).length * 7;
}
const getNodesHeight = (count) => count === 0 ? 0 : count * NODE_HEIGHT + (count - 1) * VERTICAL_GAP;
function measureGroup(kind, nodes, measureLabel) {
    const sortedNodes = sortByLabel(nodes);
    const nodeWidth = Math.max(NODE_MIN_WIDTH, ...sortedNodes.map(({ label }) => measureLabel(label) + NODE_LABEL_PADDING * 2));
    return {
        kind,
...
function resolveEdges(edges, nodeById) {
...
export function layoutComponentMap(data) {
    const nodesByKind = new Map(KIND_ORDER.map((kind) => [kind, []]));
    for (const node of data.nodes) {
        nodesByKind.get(node.kind)?.push(node);
    }
    const measureLabel = createLabelMeasurer();
    const measuredGroups = KIND_ORDER.map((kind) => measureGroup(kind, nodesByKind.get(kind) ?? [], measureLabel));
    ...
    const provisionalBands = measuredGroups.map((group) => createBand(group, 0, commonCenterY - group.visibleHeight / 2));
    ...
    const corridorByKind = new Map(KIND_ORDER.map((kind) => [kind, 0]));
    for (const edge of provisionalEdges) {
        if (edge.source.kind === edge.target.kind) {
            corridorByKind.set(edge.source.kind, Math.max(corridorByKind.get(edge.source.kind) ?? 0, getSameGroupArcOutset(edge.source, edge.target) + ARROW_SIZE));
        }
    }
    ...
    const rightmost = rightmostBand.bounds.x +
...
export function hitTest(layoutOrNodes, point) {
