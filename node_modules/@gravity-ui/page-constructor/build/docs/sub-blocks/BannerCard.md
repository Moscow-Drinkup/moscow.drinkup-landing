...
> The schema for this sub-block is defined in `src/blocks/Banner/schema.ts` (`BannerCardProps`) and shared with the standalone `banner-block`.
...
| `button` _required_ | `ButtonProps`             | —         | Single call-to-action button. Uses only `text`, `url`, `target`, `theme` (`'raised'` by default). |
